...
- (void)changeSubviewFrame {
    ...
    CGFloat bottomViewY = height - 50 -150 - bottomMargin;
...
- (HXDatePhotoBottomView *)bottomView {
    if (!_bottomView) {
        _bottomView = [[HXDatePhotoBottomView alloc] initWithFrame:CGRectMake(0, self.view.hx_h - 50 -150  - kBottomMargin, self.view.hx_w, 50 + kBottomMargin)];
...
- (UICollectionView *)collectionView {
    if (!_collectionView) {
        CGFloat collectionHeight = self.view.hx_h - 150;
        if (self.manager.configuration.showDateSectionHeader) {
